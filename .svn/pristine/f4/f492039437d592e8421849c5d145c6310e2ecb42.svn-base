...
class ListServiceImpl(
  ...
  def list(params: CommonParameters): FinalResult = Stats.time("srv:list") {
    inTransaction {
      ...
      listStart(origin, content, pager, sortKey, debug)
    }
  }
  
  def shopIdList(params: CommonParameters): Result = Stats.time("srv:shopIdlist") {
    inTransaction {
      import params._
      val (city, district, bizone) = regionService.resolve(cityId, districtId, bizoneId)
      val origin = QueryParams(
      ...
      getShopIdForDPStart(origin, pager, sortKey, debug)
    }
  }
  ...
  private def listStart(
    ...
    val dealType = if (List("30000001", "30000004", "30000005", "30000006").contains(origin.subType.headOption.getOrElse(""))) {
      "dlNoSp"
  ...
  private def allShopList(origin: QueryParams, pager: Pager, sortKey: String) = {
    ...
    FinalResult(
      ...
      categoryCounts(origin, shopCountService).toArray.map(_.toMap),
      districtCounts(origin, shopCountService).toArray.map(_.toMap),
      dealTypeCounts(origin, shopCountService).toArray.map(_.toMap),
      bankCounts(origin, shopCountService).toArray.map(_.toMap),
      brandCounts(origin, shopCountService).toArray.map(_.toMap),
      mallCounts(origin, shopCountService).toArray.map(_.toMap)
    )
  }
  ...
  def dealShopList(origin: QueryParams, pager: Pager) = {
    ...
    FinalResult(
      Result(Pager.numOfPages(totalCount, pager.size), totalCount, itemsWithShops.map(_.toMap)),
      categoryCounts(origin, dealCountService).toArray.map(_.toMap),
      districtCounts(origin, dealCountService).toArray.map(_.toMap),
      dealTypeCounts(origin, dealCountService).toArray.map(_.toMap),
      bankCounts(origin, dealCountService).toArray.map(_.toMap),
      brandCounts(origin, dealCountService).toArray.map(_.toMap),
      mallCounts(origin, dealCountService).toArray.map(_.toMap)
    )
  }
  ...
  private def brandList(origin: QueryParams, pager: Pager, sortKey: String) = {
    ...
    FinalResult(
      Result(Pager.numOfPages(totalCount, pager.size), totalCount, itemsWithDeals.map(_.toMap)),
      categoryCounts(origin, shopCountService).toArray.map(_.toMap),
      districtCounts(origin, shopCountService).toArray.map(_.toMap),
      dealTypeCounts(origin, shopCountService).toArray.map(_.toMap),
      shopCountsByBank(origin, shopCountService).toArray.map(_.toMap),
      brandCounts(origin, shopCountService).toArray.map(_.toMap),
      mallCounts(origin, shopCountService).toArray.map(_.toMap)
    )
  }
  ...
  private def bankList(origin: QueryParams, pager: Pager, sortKey: String) = {
    ...
    FinalResult(
      Result(Pager.numOfPages(totalCount, pager.size), totalCount, items.slice(pager.skip, pager.skip + pager.size).values.toList.map(_.toMap)),
      categoryCounts(origin, shopCountService).toArray.map(_.toMap),
      districtCounts(origin, shopCountService).toArray.map(_.toMap),
      dealTypeCounts(origin, shopCountService).toArray.map(_.toMap),
      shopCountsByBank(origin, shopCountService).toArray.map(_.toMap),
  ...
  def aggregate(query: QueryParams, sortKey: String, shopOnly: Boolean = false) = Stats.time("srv:aggregate") {
