...
class Invocation[T, R] private (val sig: (T => R, T, Long))
...
object Invocation {
  ...
  private val map = new WeakHashMap[(_ => _, _, Long), WeakReference[Invocation[_, _]]]
  
  private def valueFromKey[T, R](k: (T => R, T, Long)) = new Invocation((k))
  private def keyFromValue[T, R](v: Invocation[T, R]) = Some(v.sig)
  
  def apply[T, R](name: (T => R, T, Long)): Invocation[T, R] = {
  ...
  case class Result[R](timestamp: Long, value: R)
}
